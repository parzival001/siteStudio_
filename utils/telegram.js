...
async function enviarMensagem(mensagem, parseMode = 'Markdown') {
  if (!mensagem || mensagem.trim() === '') return console.warn('⚠️ Mensagem vazia para admin.');
...
async function enviarMensagemAluno(mensagem, parseMode = 'Markdown') {
  if (!mensagem || mensagem.trim() === '') return console.warn('⚠️ Mensagem vazia para alunos.');
